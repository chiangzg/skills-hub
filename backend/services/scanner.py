...
class SkillScanner:
    ...
    def _build_raw_url(self, repo: Repository, directory: str) -> str:
        """构建文件预览 URL（blob 用于页面预览）"""
        if repo.type == RepositoryType.GITHUB:
            return f"https://github.com/{repo.full_name}/blob/{repo.branch}/{directory}/SKILL.md"
        else:  # GITLAB
            base_url = repo.gitlab_url or "https://gitlab.com"
            return f"{base_url}/{repo.full_name}/blob/{repo.branch}/{directory}/SKILL.md"
